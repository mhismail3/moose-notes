...
> Rationale: Enables scalable, cost-efficient collaboration between multiple AI models while preserving a single, verifiable source of truth.

1. **Roles**
   - **Central Agent** – a higher-capacity “thinking” model responsible for  
     a. decomposing PBIs into atomic tasks,  
     b. creating *task-assignment prompts* in markdown, and  
     c. maintaining the *Memory Bank* (see below).
   - **Worker Agent** – a token-efficient model that consumes a single task-assignment prompt, executes the task, and writes back its result or any encountered bug.

2. **Memory Bank**
   - Implemented as `docs/delivery/memory-bank.md` **or** one log file per task (e.g., `tasks/<PBI-ID>-<TASK-ID>-log.md`).  
   - Each entry **must** reference the originating task file and include:
     `| Timestamp | Task ID | Status (success/blocked) | Notes/Bug | Author |`
   - Central Agent consults this log before generating any new task-assignment prompt.

3. **Execution Loop**
   1. Central Agent generates a markdown *task-assignment prompt* for the next atomic task.  
   2. Worker Agent performs the task and appends its outcome to the Memory Bank.  
   3. Central Agent reviews the updated Memory Bank:  
      - If status = **blocked**, emit a follow-up prompt or create a new task to unblock.  
      - If status = **success**, proceed to the next planned task.  
   4. Repeat until all tasks under the PBI are **Done**.

4. **Cost Optimisation**
   - Prefer high-capacity models **only** for Central Agent reasoning; delegate atomic tasks to smaller models (e.g., GPT-4-mini) whenever quality and token limits permit.

5. **Compliance**
   - All tasks and Memory Bank entries must still follow §§ 4.4–4.10 regarding status synchronisation and logging.  
   - The Memory Bank does **not** replace individual task history logs; it complements them as the cross-task narrative.

_Note:_ This framework is optional; it applies when multiple AI agents are used. Single-agent workflows remain fully compliant with the existing policy.
